...
CREATE TABLE Game (
	...
	designerid NUMBER(6) NOT NULL REFERENCES Person.id);

CREATE TABLE Rating (
	value NUMBER(2) NOT NULL,
	userid NUMBER(6) NOT NULL REFERENCES Users.id,
	gameid NUMBER(6) NOT NULL REFERENCES Game.id,
...
CREATE TABLE GamePublisher (
	gameid NUMBER(6) NOT NULL REFERENCES Game.id,
	publisherid NUMBER(6) NOT NULL REFERENCES Publisher.id,
	PRIMARY KEY (gameid, publisherid));

CREATE TABLE GameArtist (
	gameid NUMBER(6) NOT NULL REFERENCES Game.id,
	artistid NUMBER(6) NOT NULL REFERENCES Person.id,
...
CREATE TABLE GameCategory (
	gameid NUMBER(6) NOT NULL REFERENCES Game.id,
	categoryid NUMBER(6) NOT NULL REFERENCES Category.id,
	PRIMARY KEY (gameid, categoryid));

CREATE TABLE GameMechanism (
	gameid NUMBER(6) NOT NULL REFERENCES Game.id,
	mechanismid  NUMBER(6) NOT NULL REFERENCES Category.id,
	PRIMARY KEY (gameid, mechanismid));

CREATE TABLE GameFamily (
	gameid NUMBER(6) NOT NULL REFERENCES Game.id,
	familyid NUMBER(6) NOT NULL REFERENCES Category.id,
